  	...
  	<table class="table">
  		...
  		<tr>
  			<td colspan="4" class="text-right inline">
  				<input type=button class="btn btn-xs btn-success" value="수정" @click="updateForm()">&nbsp;
  				<input type=button class="btn btn-xs btn-danger" value="삭제" ref="delBtn" @click="boardDeleteForm()">&nbsp;
  				<input type=button class="btn btn-xs btn-warning" value="목록" @click="boardList()">
  			</td>
  		</tr>
  		<tr v-show="isShow">
  			<td colspan="4" class="text-right inline">
  				비밀번호:<input type="password" ref="pwd" v-model="pwd" class="input-sm" size="15">
  				<input type="button" class="btn btn-sm btn-primary" value="삭제" @click="boardDelete()">
  			</td>
  		</tr>
  	</table>
