				           ...
				           <tr>
					        ...
					        <tr>
					         <td colspan="2" class="text-center inline" style="border-bottom: none">
					           <input type=button value="수정하기" class="btn-sm btn-danger" @click="update()" id="boardBtn">&nbsp;
...
let updateApp=Vue.createApp({
	...
	mounted(){
		axios.get('../musicboard/update_vue.do',{
			   params:{
				   no:this.no
			   }
		   }).then(response=>{
	...
	methods:{
		update(){
			...
			axios.post('../musicboard/update_ok_vue.do',null,{
				params:{
					name:this.name,
					title:this.title,
				...
				{
				   location.href="../musicboard/detail.do?no="+this.no;	
				}
				else
				{
